...
class LiO2_1D(pybamm.BaseModel):
    ...
    def __init__(self, name="Li-O2 1D"):
        ...
        a = a0 * pybamm.Scalar(0.5) * (1 - pybamm.erf((l_Li2O2 - lm) / l_sigma)) # Eq 22: a0·[1 - erf(...)]/2
        #a = a0
        ...
        dcLi2O2_dt = (1-H) * (-a * j_c / (2*F))   # Eq 6 with s=+1: R = -a·j_c/(2F)

        # 1D Electrode Clogging (Eq 20)
        deps_dt = H * (a * j_c * M_Li2O2 / (2 * F * rho_Li2O2))   # Eq 20: dε/dt = a·j_c·M/(2Fρ)

        # Lithium ion migration (Eq 4)
        N_Li = -D_Li_eff * pybamm.grad(cLi) + i_l * tP / F
        R_Li = a*j_c/F                            # Eq 6 with s=-2: R = -a·(-2)·j_c/(2F) = a·j_c/F
        dcLi_dt = ((-pybamm.div(N_Li) + R_Li) - cLi * deps_dt )/ eps_safe

        # 1D O2 Cathode Diffusion (Eq 5)
        N_O2 = -D_O2_eff * pybamm.grad(cO2)     # O2 flux
        dcO2_dt = ((-pybamm.div(N_O2) + a * j_c / (2*F)) - cO2 * deps_dt) / eps_safe  # Eq 5 with R_O2 = a·j_c/(2F)
