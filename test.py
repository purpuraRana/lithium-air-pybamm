...
class LiO2_1D(pybamm.BaseModel):
    ...
    def __init__(self, name="Li-O2 1D"):
        ...
        a = a0 * pybamm.Scalar(0.5) * (1 - pybamm.erf((l_Li2O2 - lm) / l_sigma)) # Eq 22
        cLi2O2_s = pybamm.minimum(cLi2O2, cLi2O2_max)
        ...
        j_c = 2*F*(k_a * cLi2O2_s * pybamm.exp(arg_a) - k_c * cLi**2 * cO2 * pybamm.exp(arg_c))
        ...
        a_init = a0 * pybamm.Scalar(0.5) * (1 - pybamm.erf((l_Li2O2_init - lm) / l_sigma))
        ...
        phi_l_guess = -J * (x - Lc) / kappa_eff0

        # ICs
        c_seed = 1e-10 * cLi2O2_max

        # Equilibrium overpotential from the seed concentration.
        c_seed_safe = pybamm.maximum(c_seed, pybamm.Scalar(1e-30))
        eta0 = (R*T/(2*F)) * pybamm.log(k_c * cLi0**2 * cO20 / (k_a * c_seed_safe))
        phi_s_guess = phi_l_guess + Eeq + eta0
...
with open('/home/claude/li02_model.py', 'w') as f:
...
print("=== Diagnostics ===")
print(f"y0: finite={np.all(np.isfinite(y0))}, min={np.nanmin(y0):.3e}, max={np.nanmax(y0):.3e}")

alg = m.concatenated_algebraic.evaluate(t=t0, y=y0, inputs=inputs).reshape(-1)
print(f"algebraic residual: finite={np.all(np.isfinite(alg))}, inf-norm={np.max(np.abs(alg)):.3e}")

rhs = m.concatenated_rhs.evaluate(t=t0, y=y0, inputs=inputs).reshape(-1)
print(f"rhs: finite={np.all(np.isfinite(rhs))}, min={np.nanmin(rhs):.3e}, max={np.nanmax(rhs):.3e}")

# Well-posedness check
try:
    m.check_well_posedness()
    print("well_posed: True")
except Exception as e:
    print(f"well_posed: False - {e}")

# Now try to actually solve
print("\n=== Attempting solve (t=0 to 2000s, J=0.2 A/m^2) ===")
try:
    t_eval = np.linspace(0, 2000, 200)
    solution = sim.solve(t_eval, inputs={"J": 0.2})
    print(f"SUCCESS! Solved to t={solution.t[-1]:.1f}s")
    print(f"Final porosity range: [{solution['Porosity'].entries[:,-1].min():.4f}, {solution['Porosity'].entries[:,-1].max():.4f}]")
    print(f"Final O2 conc range: [{solution['O2 concentration'].entries[:,-1].min():.4f}, {solution['O2 concentration'].entries[:,-1].max():.4f}]")
except Exception as e:
    print(f"FAILED: {e}")
    
    # Try a tiny solve
    print("\n=== Trying tiny solve (t=0 to 1e-3) ===")
    try:
        sim2 = pybamm.Simulation(
            LiO2_1D(),
            parameter_values=params,
            geometry={"cathode": {pybamm.SpatialVariable("x", domain="cathode", coord_sys="cartesian"): {"min": pybamm.Scalar(0), "max": pybamm.Parameter("Lc")}}},
            submesh_types=submesh_types,
            var_pts={pybamm.SpatialVariable("x", domain="cathode", coord_sys="cartesian"): 80},
            spatial_methods=spatial_methods,
            solver=pybamm.CasadiSolver(mode="safe", atol=1e-6, rtol=1e-6),
        )
        sol2 = sim2.solve(np.array([0, 1e-3]), inputs={"J": 0.2})
        print(f"Tiny solve SUCCESS! t_final={sol2.t[-1]}")
    except Exception as e2:
        print(f"Tiny solve FAILED: {e2}")